...
def analisar_com_treinamento(caminho_pdf):
    ...
    try:
        ...
        arquivos_esperados = {
            "referencia_branco": "BASELINE 0: Fundo vazio. Quadrado idêntico a este ou apenas com sujeira de scanner.",
            "exemplo_marcado_fraco": "LIMITE MÍNIMO DE 1: Este risco fraco é o SEU PISO. Marcação igual ou MAIS FORTE é 1.",
            "exemplo_vazado": "LIMITE DE BORDA DE 1: Risco humano cruzando os limites.",
            "exemplo_linha_fina": "LIMITE DE PRESSA: Risco fino, reto e longo cruzando a caixa. É humano. DEVE SER MARCADO.",
            "exemplo_passou_longe": "O HUMANO PREGUIÇOSO: A reta passou por cima ou ao lado do quadrado sem tocá-lo. DEVE SER MARCADO.",
            "exemplo_preguicoso_fraco": "O FANTASMA: Traço extremamente fraco/apagado, quase invisível, começando fora do quadrado. É VISTO HUMANO. DEVE SER MARCADO.",
            "referencia_raspa_fora": "O TANGENCIAL: Risco que raspa a borda externa do quadrado. Qualquer traço na região externa adjacente é humano. DEVE SER MARCADO."
        }
        ...
        st.write("☁️ Subindo lote normalizado (Documento + Exemplos de Visão)...")
        aguardar_processamento(arquivos_ia_alvo + list(arquivos_ia_treino.values()))

        telemetria_total = {"in": 0, "out": 0, "brl": 0.0}

        st.markdown("---")
        st.write("### 👁️ Leitura Dinâmica (Visão Espacial Restrita)")
        ...
        if arquivos_ia_treino:
            conteudo_prompt.append("\n--- CALIBRAGEM DE VISÃO E ESCALA ---")
            conteudo_prompt.append("Estude as imagens abaixo. ELAS FORAM NORMALIZADAS PARA A MESMA ESCALA EXATA. Use-as para entender a grossura do traço e a área de avaliação:")
            
            for key in ["referencia_branco", "exemplo_marcado_fraco", "exemplo_vazado", "exemplo_linha_fina", "exemplo_passou_longe", "exemplo_preguicoso_fraco", "referencia_raspa_fora"]:
        ...
        # --- REGRAS DE DETECÇÃO COM CHAIN OF THOUGHT (SISTEMA 2) ---
        ...
        try:
            ...
            try:
                ...
                telemetria_total["brl"] += (t_in + t_out) * 0.000001 * 5.5
            except:
                pass

            # --- O ZÍPER EM PYTHON (FUZZY MATCH) ---
            ...
            for linha in linhas:
                ...
                if linha.startswith("[X]") or linha.startswith("[x]") or linha.startswith("(X)"):
                    marcado = True
                    frase_lida = linha[3:].strip()
